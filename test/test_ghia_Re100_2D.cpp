...
TEST_CASE("Ghia 1982 Benchmark (Re=100, 2D lid-driven cavity flow)", "[ghia_re100_2d]") {
    ...
    float_type lid_velocity = 1.0; // Lid velocity = 1.0 for normalized units
    float_type Re = 100.0;
    float_type L = static_cast<float_type>(N - 1); // Domain length in lattice units
    float_type nu = lid_velocity * L / Re; // Kinematic viscosity (from Re definition)
    ...
    float_type u_center = lattice.u[0][center_idx]/lid_velocity; // u at (N/2, N/2), normalized by lid velocity 
    float_type v_center = lattice.u[1][center_idx]/lid_velocity; // v at (N/2, N/2), normalized by lid velocity
